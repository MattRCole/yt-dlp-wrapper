...
app.use(async (request, _, next) => {
  ...
  const { socket, response: socketResponse } = Deno.upgradeWebSocket(
    request.ogRequest,
  );
...
const shouldAbortDownload = (currentStatus: StatusMessage): boolean => {
  ...
  const similarStatuses = statusKeeper.getSimilarStatuses({
    ...currentStatus,
    ytID,
  }, currentStatus.id);

  const [statusesStillDownloading, notDownloading] = similarStatuses.reduce(
    ([sd, nd], fullStatus) => {
...
const baseDownloadRequestPipe = <T extends DownloadInfo>(
  body: T,
  requestId: string,
  baseStatus: Omit<StatusMessage, "status">,
): () => Promise<
  [RequireSome<YouTubeInfo, "id" | "playlistId"> | undefined, boolean]
> => {
...
router.post("/video/download", (_, ctx) => {
  ...
  const pipe = pipeWhile<[], undefined>(
    ...
    (info: RequireSome<YouTubeInfo, "id">) => {
      ...
      return youtubeDownloader.downloadVideo(
        info.id,
        body,
        (partIdx, percent) =>
          statusKeeper.handleDownloadProgress(requestId, partIdx, percent),
      ).then(([success, stdErr]) => {
        if (!success) {
          console.error({
            id: info.id,
            msg: "problem downloading video",
            stdErr,
          });
          statusKeeper.setStatus(requestId, {
            ...baseStatus,
...
router.post("/video-list/download", (_, ctx) => {
  const body = ctx?.body as
    | DownloadInfo & VideoPostOptions & ListPostOptions
    | undefined
    | null;
  ...
  const pipe = pipeWhile(
    ...
    (info: RequireSome<YouTubeInfo, "playlistId">) => {
      ...
      return youtubeDownloader.downloadVideoList(
        info.playlistId,
        body,
        (p, p2) => statusKeeper.handleDownloadProgress(requestId, p, p2),
      ).then(([success, stdErr]) => {
        if (!success) {
          console.error({
            id: info.id,
            msg: "problem downloading video playlist",
            stdErr,
          });
          statusKeeper.setStatus(requestId, {
            ...baseStatus,
...
router.post("/music/download", (_, ctx) => {
  ...
  const pipe = pipeWhile<[], undefined>(
    ...
    (info: RequireSome<YouTubeInfo, "id">) => {
      ...
      return youtubeDownloader.downloadSong(
        info.id,
        body,
        (p, p2) => statusKeeper.handleDownloadProgress(requestId, p, p2),
      ).then(([success, stdErr]) => {
...
router.post("/music-list/download", (_, ctx) => {
  const body = ctx?.body as
    | DownloadInfo & MusicPostOptions & ListPostOptions
    | undefined
    | null;
  ...
  const pipe = pipeWhile<[], undefined>(
    ...
    (info: RequireSome<YouTubeInfo, "playlistId">) => {
      ...
      return youtubeDownloader.downloadMusicList(
        info.playlistId,
        body,
        (p, p2) => statusKeeper.handleDownloadProgress(requestId, p, p2),
      ).then(([success, stdErr]) => {
        if (!success) {
          console.error({
            id: info.id,
            msg: "problem downloading music list",
            stdErr,
          });
          statusKeeper.setStatus(requestId, {
            ...baseStatus,
...
app.listen({
  port: parseInt(Deno.env.get("YTDLW_PORT") || ""),
  hostname: hostAddr,
});
